...
public class GameOfLife {
	...
	public static int cellValue(int[][] board, int i, int j) {
				int numOfLiveAround = count(board, i, j);
				int cellOldValue = board[i][j];
				int cellNewValue = 0;
				if (cellOldValue == 1){
					if (numOfLiveAround < 2){
						cellNewValue = 0;
					}
					...
					if (cellOldValue == 0 && numOfLiveAround == 3){
						cellNewValue = 1;
					}
				} 
